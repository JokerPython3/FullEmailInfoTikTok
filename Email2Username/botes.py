   ...
   def get_ksj(self):
    ...
    try:
        ...
        try:
            ...
            response = self.s.post(url, headers=headers, params=params, cookies=cookies)
            if "email_ticket" in response.text:
                ...
                # print(response.json())
                try:
                    username = response.json()[0]["body_text"].split('This email was generated for')[1].split("\n")[0].strip().rstrip(".")
       ...
       return{"status":"error","message":"ksj"}
Java = telebot.TeleBot(input("enter token bot telegram :"))
@Java.message_handler(commands=["start"])
def start(message):
   b  = btn("channel",url="https://t.me/python3_Tool")
   m = mk(row_width=1).add(b)
   Java.reply_to(message,text="<strong> send email </strong>",parse_mode="html",reply_markup=m)
@Java.message_handler(func=lambda m:True)
def Ntro(ms):
   user = ms.text
   w=Email2UsernameTikTok(user).get_ksj()
   try:
    b  = btn("channel",url="https://t.me/python3_Tool")
    m = mk(row_width=1).add(b)
    Java.send_message(ms.chat.id,text=f"<strong> username : <code>{w["username"]}</code></strong>",parse_mode="html",reply_markup=m)
   except:
     b  = btn("channel",url="https://t.me/python3_Tool")
     m = mk(row_width=1).add(b)
     Java.send_message(ms.chat.id,text="<strong> error </strong>",parse_mode="html",reply_markup=m)
Java.polling(True)
#I love Java     